...
#table(
  ...
  [*阶段*], [*内容*], [*起止时间*], [*产出*],
  [第一阶段（预备阶段）],
  [测试人员阅读本子系统的设计文档与需求文档，熟悉各个功能模块所实现的具体功能，了解本子系统各个输入数据的边界情况。同时，寻找用于测试相关的工具。],
  [TODO],
  [测试计划、测试工具清单],
  ...
  [第三阶段（测试阶段）],
  [测试人员针对已经开发出来的子系统，用测试用例对系统进行模块功能测试、边界值测试、接口测试与安全性测试，找出本子系统中存在的缺陷或错误。],
  [TODO],
  [测试执行记录、缺陷报告],

  [第四阶段（后期阶段）],
  [本子系统编码人员根据测试阶段的结果调整程序代码，修复存在的缺陷或错误。测试人员对修复后的系统进行回归测试。],
  [TODO],
  [修复记录、回归测试报告],
)
...
#table(
  ...
  [SYSTEM_ADMIN 登录],
  [GET /audit/operation-logs],
  [操作步骤：系统管理员进入审计页面。\ 预期输出：正常返回操作日志列表。],
...
#table(
  columns: (3fr, 2fr, 1fr, 1fr, 3fr),
  [*场景*], [*工具*], [*并发数*], [*持续时间*], [*验证指标*],
  [登录接口压测], [Locust / wrk], [50], [60s], [P95 < 500ms，错误率 0%],
  [股票查询压测], [Locust], [50], [60s], [P95 < 300ms],
  [错误率 < 1%], [混合操作压测（查行情+设涨跌停+查日志）], [Locust], [30], [120s],
  [P95 < 1s，无死锁], [审计日志大数据量查询], [手工 + Locust], [10], [—],
  [10万条数据下分页查询 < 1s], [CSV 大导出], [手工], [1], [—],
  [10万条导出 < 5s，内存不溢出],
)
...
#table(
  columns: (2fr, 1fr, 2fr, 2fr, 2fr, 1fr, 1fr, 1fr, 2fr),
