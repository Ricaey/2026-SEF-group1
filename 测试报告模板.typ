...
// #image("../image/600筛选.png", width: 80%)
// #image("../image/股票详情查看.png", width: 80%)
...
// #image("../image/涨跌停不填股票代码.png", width: 70%)
...
// #image("../image/密码强度引导——纯数字.png", width: 50%)
// #image("../image/密码强度引导——两种字符.png", width: 50%)
// #image("../image/密码强度引导——强度达标.png", width: 50%)
...
// #image("../image/日志页正常分页.png", width: 80%)
// #image("../image/导出CSV下载.png", width: 70%)
...
// #image("../image/MySQL 不可用时健康检查（HC-002）.png", width: 70%)
// #image("../image/TRADE 不可达.png", width: 70%)
...
// #image("../image/HTML注入.png", width: 70%)
...
// #image("../image/密码错误登录.png", width: 50%)
// #image("../image/加载动画.png", width: 50%)
// #image("../image/无数据时"暂无数据"提示.png", width: 50%)
// #image("../image/无法连接TRADE服务后显式提示.png", width: 50%)
...
#table(
  ...
  [SEC-007], [SQL注入防护 | 422/空结果 | OR注入→401; DROP/UNION→422(Pydantic拦截)], [✅ 通过], [ORM参数化查询 + Schema校验],
...
（无待补充内容）：填入各接口在不同并发下的测试结果数据。

#table(
  columns: (2fr, 1fr, 2fr, 2fr, 2fr, 1fr, 1fr, 1fr, 2fr),
  [*测试接口*], [*并发*], [*平均响应(ms)*], [*中位数(ms)*], [*P95(ms)*], [*QPS*], [*错误率*], [*是否达标*], [*备注*],
  [POST /auth/login], [未执行], [—], [—], [—], [—], [—], [—], [本阶段未执行性能测试],
  [], [未执行], [—], [—], [—], [—], [—], [—], [本阶段未执行性能测试],
  [], [未执行], [—], [—], [—], [—], [—], [—], [本阶段未执行性能测试],
  [], [未执行], [—], [—], [—], [—], [—], [—], [本阶段未执行性能测试],
  [GET /stocks], [未执行], [—], [—], [—], [—], [—], [—], [本阶段未执行性能测试],
  [], [未执行], [—], [—], [—], [—], [—], [—], [本阶段未执行性能测试],
  [], [未执行], [—], [—], [—], [—], [—], [—], [本阶段未执行性能测试],
  [GET /audit/operation-logs], [未执行], [—], [—], [—], [—], [—], [—], [本阶段未执行性能测试],
  [], [未执行], [—], [—], [—], [—], [—], [—], [本阶段未执行性能测试],
  [GET /audit/logs/export], [未执行], [—], [—], [—], [—], [—], [—], [本阶段未执行性能测试],
)
